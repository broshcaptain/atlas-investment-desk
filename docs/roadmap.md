...
- Sabah brifingi için gerçek LLM entegrasyonu: `ai/summaries/morning_briefing.py:generate_morning_briefing()` LLM API'ye bağlandı. API anahtarı tanımlı değilse `anahtar_eksik`, API hatası olursa `hata` döner — hiçbir durumda sahte metin üretilmez. `backend/services/dashboard_service.py`, TUPRS `company_overview`'ini de çekip `/dashboard` yanıtına `briefing` alanı olarak ekliyor; `frontend/components/BriefingCard.tsx` gerçek brifing metnini (veya durum mesajını) gösteriyor. Uçtan uca canlı doğrulandı — VERİ BAYAT uyarısı, "olabilir" dili, güven skoru bant+parantez formatı ve al/sat tavsiyesi içermeme kuralı üretilen metinde doğru uygulandı.
- KAP duyuru içeriği temizliği: `collectors/kap/common.py:fetch_disclosure_text()` artık ham XBRL taksonomi etiketlerini (`taxonomy-field-name-cell` — ör. `oda_ExplanationTextBlock|`) kaldırıyor ve tablo yapısını taksonomiye özgü sınıf adlarına bağlı kalmadan "Etiket: değer" biçiminde satır satır okunabilir metne çeviriyor; tablo bulunamazsa düz metne geri dönüyor. Gerçek TUPRS duyurularıyla (Finansal Takvim, Özel Durum Açıklaması) canlı doğrulandı. Yeni `ai/summaries/announcement_summary.py:generate_announcement_summary()` bu temiz metni LLM ile 1-3 cümlelik tarafsız özete çeviriyor (aynı `anahtar_eksik`/`hata`/sahte özet üretmeme kalıbı); `collectors/kap/tuprs_announcements.py` her yeni duyuru için bunu çağırıp `ai_summary`'yi dolduruyor, başarısız olursa `ai_summary` `NULL` kalıyor. Gerçek anahtarla iki gerçek TUPRS duyurusu üzerinde uçtan uca canlı doğrulandı — özetler doğru ve kaynak metne sadık.
...
- Dil sızıntısı düzeltmesi: model `temperature=0.3`'te bile Türkçe metne ara sıra Çince/İngilizce kelime karıştırmaya devam etti (canlı kullanımda gözlemlendi). İki sistem promptuna da en üste açık bir "DİL" kuralı eklendi (sadece Türkçe, başka dilden tek kelime/karakter bile yasak) ve `temperature` 0.3'ten 0.1'e düşürüldü. Ayrıca `ai/summaries/language_guard.py:find_non_turkish_script()` eklendi — CJK/Hiragana-Katakana/Hangul/Arapça/Kiril/Tayca/Devanagari kod noktası aralıklarını tarayan runtime koruma katmanı; `generate_morning_briefing()` ve `generate_announcement_summary()` model yanıtını döndürmeden önce bu taramadan geçiriyor, sızıntı tespit edilirse `hata` döner (sonuç gösterilmez). Gerçek anahtarla 5+5 ardışık canlı çağrıda hiçbirinde yabancı dil karışması görülmedi; guard'ın kendisi de mock kirli yanıtla tetiklendiği doğrulanarak test edildi.

- TUPRS finansal verisine gerçek ikinci kaynak: kullanıcı, tek kaynaklı veriye otomatik "Düşük" güven bandı verilmesine itiraz etti. İş Yatırım'ın (isyatirim.com.tr) kamuya açık finansal tablo endpoint'i araştırılıp entegre edildi. Yeni `collectors/kap/tuprs_financials_isyatirim.py`, yfinance'ten (birincil kaynak) SONRA çalışıp aynı `company_financials` satırını ham bilanço/gelir tablosu kalemleriyle çapraz kontrol ediyor; `roe/roic/debt/cash` sütunlarına dokunmuyor, sadece yeni `source_count`/`has_conflicting_data` kolonlarını (`database/migrations/0002_add_financials_source_tracking.sql`) dolduruyor. `backend/services/company_service.py` bu gerçek değerleri `analyze_company()`'e geçiriyor. Canlı doğrulandı: ROE/ROIC'te gerçek bir sapma bulundu (%8.1 vs %10.4, %9.1 vs %14.2), `has_conflicting_data=True` çıktı, güven ham skoru 57.1'den 28.6'ya düştü — band hâlâ "Düşük" ama artık "kaynaklar çelişiyor" gerekçesiyle. Temettü verimi kapsam dışı (İş Yatırım endpoint'inde yok), hâlâ yfinance-only.
- Güven skoru açıklanabilirliği: kullanıcı "Güven: Düşük (29/100)"in tek başına anlaşılır olmadığını belirtti. `ai/analyzers/company_analyzer.py:_confidence()` artık tek cümlelik somut bir `reason` alanı döndürüyor, `frontend/components/AtlasScoreCard.tsx` bunu rozetin altında gösteriyor. `collectors/kap/tuprs_financials_isyatirim.py`'deki kaynak karşılaştırma notu da ham float/pseudocode yerine biçimlendirilmiş yüzde/TL değerleri (`%8.1 vs %10.4`) kullanacak şekilde düzeltildi.
...
- İş Yatırım'ın `MaliTablo` endpoint'i resmi/dokümante değil — düşük hacimli/nazik kullanım gerekiyor, URL/parametre şekli habersiz değişebilir.
